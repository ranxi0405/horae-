...
function initSettingsEvents() {
    ...
    $('#horae-setting-auto-summary-keep').on('change', function () {
        settings.autoSummaryKeepRecent = Math.max(0, parseInt(this.value) || 10);
...
async function onPromptReady(eventData) {
    ...
    try {
        ...
        /*
         * 动态物品检索使用本次最终提示词链路中的 USER 输入。
         *
         * 不从 horaeManager.getChat() 获取当前输入。
         * getChat() 继续只用于 Horae 已保存的历史状态。
         *
         * 这样保持当前“关闭酒馆聊天记录后仍能识别玩家输入”的
         * 现有提示词链路，不重新引入旧问题。
         *
         * content 兼容字符串与数组两种形态（数组为多模态消息格式）。
         */
...
const combinedPrompt =
    ...
    if (timelinePrompt) {
            ...
            // Horae固定规则插入系统区末尾，保持缓存稳定
const stableIdx = eventData.chat.findIndex(
    m => m.role === 'system' && m.content?.trim() === '[Start a new Chat]'
);

const fixedInsertIdx = stableIdx >= 0 ? stableIdx + 1 : 0;

eventData.chat.splice(fixedInsertIdx, 0, {
    role: 'system',
    content: stableRulesPrompt
});

// 固定规则插入后，重新计算动态注入位置
const dynamicInsertIdx = _resolveInsertIndexByChatAnchor(
    chat,
    eventData.chat,
    position
);
...
// 动态数据 + 反转述内容
eventData.chat.splice(dynamicInsertIdx, 0, {
    role: 'system',
    content: combinedPrompt
});

window.horaeDebugPrompt = combinedPrompt;
window.horaeDebugChat = eventData.chat;
